...
def resolve_target_repo(target: Path) -> tuple[Path, Path]:
    ...
    if target.name == "development_skills" and target.exists():
...
def pick_twin_root(repo_name: str, preferred_central: Path, ds: Path) -> Path:
    ...
    for root in roots:
        twin = root / "39_repo_twins" / "twins" / repo_name
        if twin.exists():
            return root
...
def tier1(repo_root: Path, ds: Path, central: Path, repo_name: str) -> list[Check]:
    ...
    sync_script = ds / "25_automation" / "registry_sync" / "sync_registries.py"
    ...
    twin = twin_root / "39_repo_twins" / "twins" / repo_name
    twin_files = [
...
def tier2(ds: Path, repo_name: str) -> list[Check]:
    ...
    starter = [p for p in find_files(starter_dir, "*.yaml") if repo_name in p.name.lower()]
...
def tier4(ds: Path) -> list[Check]:
    ...
    ledger_dir = ds / "13_skills" / "skill_refinery" / "mistake_ledgers"
    ...
    imp_dir = ds / "13_skills" / "skill_refinery" / "improvement_proposals"
