...
- 2026-05-04 (v1.5): Added Clauses C-15 through C-21 — executable
  ...
  **displacement thesis**: SC has no weaknesses by design — Volume
  ...
  scope: all `.claude/` artifacts live at `superc-v1/.claude/`,
  never `~/.claude/`.
...
agent memory, settings — live at `superc-v1/.claude/`. They
do **NOT** live at `~/.claude/`. Authors editing this skill,
adding sibling skills, or wiring hooks must target the
project-local path. Cross-contamination with the user-global
`~/.claude/` tree is forbidden without explicit per-request
permission.
