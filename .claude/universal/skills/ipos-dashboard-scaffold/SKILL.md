---
name: ipos-dashboard-scaffold
description: "IPOS Dashboard Scaffold — Universal dashboard scaffolding skill: generates complete project structures for React/Next.js, Vue/Nuxt, Angular, Svelte/SvelteKit, Python (Dash/Streamlit), an"
source: platform/sdlc/13_skills/active/SKILL_IPOS_DASHBOARD_SCAFFOLD_001.yaml
canonical_id: SKILL_IPOS_DASHBOARD_SCAFFOLD_001
generated: 2026-05-27
runtime: codex
---

> **Runtime projection of `SKILL_IPOS_DASHBOARD_SCAFFOLD_001`.** Edit the canonical, not this file.
> Source of truth: `platform/sdlc/13_skills/active/SKILL_IPOS_DASHBOARD_SCAFFOLD_001.playbook.md`

# IPOS Dashboard Scaffold Playbook

## Purpose
Universal dashboard scaffolding skill: generates complete project structures for React/Next.js, Vue/Nuxt, Angular, Svelte/SvelteKit, Python (Dash/Streamlit), and Vanilla JS dashboards. Includes build configs, linting, testing, CI/CD, environment management, and folder conventions. Trigger on: 'scaffold', 'new dashboard', 'project setup', 'folder structure', 'project structure', 'initialize project', 'create dashboard'.

## Imported Source
- Raw source: `16_knowledge/external_collateral/ipos_claude_skills_2026-05-17/raw/dashboard-scaffold/SKILL.md`
- Source repo: `/Users/desmondearly/Developer/IPOS`
- Raw SHA-256: `689e46afb7644bce70ac35146614b021ec26f2e9d71113c328c5eba7cd7e0eb3`

## Activation Rule
Use this skill when the request matches the imported trigger language, the IPOS/paratransit dashboard domain, or the user asks for the named capability: `dashboard-scaffold`.

## Operating Contract
- Apply the imported patterns from the raw `SKILL.md` before inventing new dashboard, KPI, SharePoint, export, testing, deployment, or UI patterns.
- Keep outputs implementation-ready: include files, schemas, components, formulas, tests, commands, or validation gates when relevant.
- Preserve auditability for KPI, contract, paratransit, SharePoint, and data pipeline outputs.
- If the request is outside the dashboard/IPOS domain, route through the trigger router first and use this skill only as a supporting branch.

## Required Output Shape
- Situation or objective.
- Applicable imported patterns.
- Implementation steps or artifact structure.
- Validation checks.
- Risks or assumptions.

## Source Detail
Read the raw source file above for the full imported body and examples when this skill fires.
