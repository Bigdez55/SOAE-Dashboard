---
name: ipos-courier
description: "IPOS Courier — Data export systems for dashboards: PDF generation using jsPDF and html2canvas, Excel export with SheetJS/xlsx, CSV download, ApexCharts dataURI chart snapshots"
source: platform/sdlc/13_skills/active/SKILL_IPOS_COURIER_001.yaml
canonical_id: SKILL_IPOS_COURIER_001
generated: 2026-05-27
runtime: codex
---

> **Runtime projection of `SKILL_IPOS_COURIER_001`.** Edit the canonical, not this file.
> Source of truth: `platform/sdlc/13_skills/active/SKILL_IPOS_COURIER_001.playbook.md`

# IPOS Courier Playbook

## Purpose
Data export systems for dashboards: PDF generation using jsPDF and html2canvas, Excel export with SheetJS/xlsx, CSV download, ApexCharts dataURI chart snapshots, and print stylesheets. Covers filename conventions, multi-sheet Excel reports, and exporting KPI data with formatting. Trigger on: "PDF export", "Excel download", "export", "print layout", "report generation", "jsPDF", "SheetJS".

## Imported Source
- Raw source: `16_knowledge/external_collateral/ipos_claude_skills_2026-05-17/raw/courier/SKILL.md`
- Source repo: `/Users/desmondearly/Library/CloudStorage/OneDrive-Personal/IPOS/IPOS`
- Raw SHA-256: `dc5faf24bfccde70c6fa5cac964289d7ff8e51b36765d1725f68a27d439e67da`

## Activation Rule
Use this skill when the request matches the imported trigger language, the IPOS/paratransit dashboard domain, or the user asks for the named capability: `courier`.

## Operating Contract
- Apply the imported patterns from the raw `SKILL.md` before inventing new dashboard, KPI, SharePoint, export, testing, deployment, or UI patterns.
- Keep outputs implementation-ready: include files, schemas, components, formulas, tests, commands, or validation gates when relevant.
- Preserve auditability for KPI, contract, paratransit, SharePoint, and data pipeline outputs.
- If the request is outside the dashboard/IPOS domain, route through the trigger router first and use this skill only as a supporting branch.

## Required Output Shape
- Situation or objective.
- Applicable imported patterns.
- Implementation steps or artifact structure.
- Validation checks.
- Risks or assumptions.

## Source Detail
Read the raw source file above for the full imported body and examples when this skill fires.
