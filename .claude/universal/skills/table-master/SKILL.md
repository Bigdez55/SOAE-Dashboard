---
name: table-master
description: "IPOS Table Master — Advanced data table patterns: TanStack Table v8, Fluent UI DetailsList, AG Grid. Sorting, filtering, pagination, virtual scrolling for 100K+ rows, column resizi"
source: platform/sdlc/13_skills/active/SKILL_IPOS_TABLE_MASTER_001.yaml
canonical_id: SKILL_IPOS_TABLE_MASTER_001
generated: 2026-05-27
runtime: codex
---

> **Runtime projection of `SKILL_IPOS_TABLE_MASTER_001`.** Edit the canonical, not this file.
> Source of truth: `platform/sdlc/13_skills/active/SKILL_IPOS_TABLE_MASTER_001.playbook.md`

# IPOS Table Master Playbook

## Purpose
Advanced data table patterns: TanStack Table v8, Fluent UI DetailsList, AG Grid. Sorting, filtering, pagination, virtual scrolling for 100K+ rows, column resizing, row selection, cell editing, export integration, sticky headers, responsive design, and ARIA grid accessibility. Trigger on: 'data table', 'DetailsList', 'TanStack Table', 'sortable table', 'filterable table', 'virtual scroll table', 'AG Grid'.

## Imported Source
- Raw source: `16_knowledge/external_collateral/ipos_claude_skills_2026-05-17/raw/table-master/SKILL.md`
- Source repo: `/Users/desmondearly/Library/CloudStorage/OneDrive-Personal/IPOS/IPOS`
- Raw SHA-256: `7cb9687a89f323f169ea47185b42886cfabc8351c61ae9d18f08b25209bdd121`

## Activation Rule
Use this skill when the request matches the imported trigger language, the IPOS/paratransit dashboard domain, or the user asks for the named capability: `table-master`.

## Operating Contract
- Apply the imported patterns from the raw `SKILL.md` before inventing new dashboard, KPI, SharePoint, export, testing, deployment, or UI patterns.
- Keep outputs implementation-ready: include files, schemas, components, formulas, tests, commands, or validation gates when relevant.
- Preserve auditability for KPI, contract, paratransit, SharePoint, and data pipeline outputs.
- If the request is outside the dashboard/IPOS domain, route through the trigger router first and use this skill only as a supporting branch.

## Required Output Shape
- Situation or objective.
- Applicable imported patterns.
- Implementation steps or artifact structure.
- Validation checks.
- Risks or assumptions.

## Source Detail
Read the raw source file above for the full imported body and examples when this skill fires.
