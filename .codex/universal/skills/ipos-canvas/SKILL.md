---
name: ipos-canvas
description: "IPOS Canvas — Custom D3.js visualizations with Codename CANVAS. Low-level SVG and Canvas rendering, force-directed graphs, custom chart types unavailable in standard React/Vu"
source: platform/sdlc/13_skills/active/SKILL_IPOS_CANVAS_001.yaml
canonical_id: SKILL_IPOS_CANVAS_001
generated: 2026-05-27
runtime: codex
---

> **Runtime projection of `SKILL_IPOS_CANVAS_001`.** Edit the canonical, not this file.
> Source of truth: `platform/sdlc/13_skills/active/SKILL_IPOS_CANVAS_001.playbook.md`

# IPOS Canvas Playbook

## Purpose
Custom D3.js visualizations with Codename CANVAS. Low-level SVG and Canvas rendering, force-directed graphs, custom chart types unavailable in standard React/Vue chart libraries, animated transitions, brush and zoom interactions, and binding D3 to React/Angular component lifecycles. Trigger on: "D3", "custom visualization", "SVG animation", "force graph", "canvas rendering", "CANVAS", "d3.js".

## Imported Source
- Raw source: `16_knowledge/external_collateral/ipos_claude_skills_2026-05-17/raw/canvas/SKILL.md`
- Source repo: `/Users/desmondearly/Developer/IPOS`
- Raw SHA-256: `cc384c45841211e8332f68b86574bf3d7a25ce2e65d376978b1ac717309b1a9a`

## Activation Rule
Use this skill when the request matches the imported trigger language, the IPOS/paratransit dashboard domain, or the user asks for the named capability: `canvas`.

## Operating Contract
- Apply the imported patterns from the raw `SKILL.md` before inventing new dashboard, KPI, SharePoint, export, testing, deployment, or UI patterns.
- Keep outputs implementation-ready: include files, schemas, components, formulas, tests, commands, or validation gates when relevant.
- Preserve auditability for KPI, contract, paratransit, SharePoint, and data pipeline outputs.
- If the request is outside the dashboard/IPOS domain, route through the trigger router first and use this skill only as a supporting branch.

## Required Output Shape
- Situation or objective.
- Applicable imported patterns.
- Implementation steps or artifact structure.
- Validation checks.
- Risks or assumptions.

## Source Detail
Read the raw source file above for the full imported body and examples when this skill fires.
