---
name: kpi-card-factory
description: "IPOS Kpi Card Factory — KPI card component factory: status indicators (Critical/Warning/On Target/Incentive), trend arrows, sparklines, penalty/incentive amounts, contract threshold ba"
source: platform/sdlc/13_skills/active/SKILL_IPOS_KPI_CARD_FACTORY_001.yaml
canonical_id: SKILL_IPOS_KPI_CARD_FACTORY_001
generated: 2026-05-27
runtime: codex
---

> **Runtime projection of `SKILL_IPOS_KPI_CARD_FACTORY_001`.** Edit the canonical, not this file.
> Source of truth: `platform/sdlc/13_skills/active/SKILL_IPOS_KPI_CARD_FACTORY_001.playbook.md`

# IPOS Kpi Card Factory Playbook

## Purpose
KPI card component factory: status indicators (Critical/Warning/On Target/Incentive), trend arrows, sparklines, penalty/incentive amounts, contract threshold bars, count-up animations, responsive grid, accessibility (ARIA, screen reader), and print optimization. Supports React, Vue, Svelte, vanilla JS. Trigger on: 'KPI card', 'metric card', 'status chip', 'KPI component', 'dashboard card'.

## Imported Source
- Raw source: `16_knowledge/external_collateral/ipos_claude_skills_2026-05-17/raw/kpi-card-factory/SKILL.md`
- Source repo: `/Users/desmondearly/Developer/IPOS`
- Raw SHA-256: `c62ec6aee12c720a6fed6cb0ba284975f10010d625b3affc5fcb4817ed76f4fb`

## Activation Rule
Use this skill when the request matches the imported trigger language, the IPOS/paratransit dashboard domain, or the user asks for the named capability: `kpi-card-factory`.

## Operating Contract
- Apply the imported patterns from the raw `SKILL.md` before inventing new dashboard, KPI, SharePoint, export, testing, deployment, or UI patterns.
- Keep outputs implementation-ready: include files, schemas, components, formulas, tests, commands, or validation gates when relevant.
- Preserve auditability for KPI, contract, paratransit, SharePoint, and data pipeline outputs.
- If the request is outside the dashboard/IPOS domain, route through the trigger router first and use this skill only as a supporting branch.

## Required Output Shape
- Situation or objective.
- Applicable imported patterns.
- Implementation steps or artifact structure.
- Validation checks.
- Risks or assumptions.

## Source Detail
Read the raw source file above for the full imported body and examples when this skill fires.
