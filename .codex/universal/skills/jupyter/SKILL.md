---
name: jupyter
description: "IPOS Jupyter — Python dashboard engineering with Codename JUPYTER. Covers Plotly Dash multi-page apps with callbacks, Streamlit rapid prototyping with session state, Panel/Hol"
source: platform/sdlc/13_skills/active/SKILL_IPOS_JUPYTER_001.yaml
canonical_id: SKILL_IPOS_JUPYTER_001
generated: 2026-05-27
runtime: codex
---

> **Runtime projection of `SKILL_IPOS_JUPYTER_001`.** Edit the canonical, not this file.
> Source of truth: `platform/sdlc/13_skills/active/SKILL_IPOS_JUPYTER_001.playbook.md`

# IPOS Jupyter Playbook

## Purpose
Python dashboard engineering with Codename JUPYTER. Covers Plotly Dash multi-page apps with callbacks, Streamlit rapid prototyping with session state, Panel/HoloViz multi-library rendering, Bokeh interactive plots, Altair declarative grammar, FastAPI hybrid backends, and production Gunicorn/Docker deployments. Trigger on: "Python dashboard", "Plotly Dash", "Streamlit app", "Panel dashboard", "Bokeh", "Altair chart", "FastAPI dashboard", "JUPYTER".

## Imported Source
- Raw source: `16_knowledge/external_collateral/ipos_claude_skills_2026-05-17/raw/jupyter/SKILL.md`
- Source repo: `/Users/desmondearly/Library/CloudStorage/OneDrive-Personal/IPOS/IPOS`
- Raw SHA-256: `8141caf12ebeb71200f8556426faf5c1ad2c527bf4c18c1a5dc0315c3748dba4`

## Activation Rule
Use this skill when the request matches the imported trigger language, the IPOS/paratransit dashboard domain, or the user asks for the named capability: `jupyter`.

## Operating Contract
- Apply the imported patterns from the raw `SKILL.md` before inventing new dashboard, KPI, SharePoint, export, testing, deployment, or UI patterns.
- Keep outputs implementation-ready: include files, schemas, components, formulas, tests, commands, or validation gates when relevant.
- Preserve auditability for KPI, contract, paratransit, SharePoint, and data pipeline outputs.
- If the request is outside the dashboard/IPOS domain, route through the trigger router first and use this skill only as a supporting branch.

## Required Output Shape
- Situation or objective.
- Applicable imported patterns.
- Implementation steps or artifact structure.
- Validation checks.
- Risks or assumptions.

## Source Detail
Read the raw source file above for the full imported body and examples when this skill fires.
