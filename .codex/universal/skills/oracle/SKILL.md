---
name: oracle
description: "IPOS Oracle — Integrating AI and ML capabilities into dashboards: Claude API streaming responses, embeddings for semantic search, confidence scoring, recommendation UI patter"
source: platform/sdlc/13_skills/active/SKILL_IPOS_ORACLE_001.yaml
canonical_id: SKILL_IPOS_ORACLE_001
generated: 2026-05-27
runtime: codex
---

> **Runtime projection of `SKILL_IPOS_ORACLE_001`.** Edit the canonical, not this file.
> Source of truth: `platform/sdlc/13_skills/active/SKILL_IPOS_ORACLE_001.playbook.md`

# IPOS Oracle Playbook

## Purpose
Integrating AI and ML capabilities into dashboards: Claude API streaming responses, embeddings for semantic search, confidence scoring, recommendation UI patterns, and client-side inference. Covers prompt engineering for KPI analysis, streaming text into dashboard panels, and displaying AI-generated insights with appropriate uncertainty indicators. Trigger on: "Claude API", "AI integration", "ML model", "streaming", "embeddings in dashboard", "LLM response".

## Imported Source
- Raw source: `16_knowledge/external_collateral/ipos_claude_skills_2026-05-17/raw/oracle/SKILL.md`
- Source repo: `/Users/desmondearly/Developer/IPOS`
- Raw SHA-256: `4ed5136659a2d85c0eb4159bcf4c44d549cb6886d747cda8074d2f9ba0869b9c`

## Activation Rule
Use this skill when the request matches the imported trigger language, the IPOS/paratransit dashboard domain, or the user asks for the named capability: `oracle`.

## Operating Contract
- Apply the imported patterns from the raw `SKILL.md` before inventing new dashboard, KPI, SharePoint, export, testing, deployment, or UI patterns.
- Keep outputs implementation-ready: include files, schemas, components, formulas, tests, commands, or validation gates when relevant.
- Preserve auditability for KPI, contract, paratransit, SharePoint, and data pipeline outputs.
- If the request is outside the dashboard/IPOS domain, route through the trigger router first and use this skill only as a supporting branch.

## Required Output Shape
- Situation or objective.
- Applicable imported patterns.
- Implementation steps or artifact structure.
- Validation checks.
- Risks or assumptions.

## Source Detail
Read the raw source file above for the full imported body and examples when this skill fires.
